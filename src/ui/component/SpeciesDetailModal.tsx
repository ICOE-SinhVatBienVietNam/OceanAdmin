...
const SpeciesDetailModal: React.FC<SpeciesDetailModalProps> = ({
  ...
  const thumbnailUrl =
    mainThumbnail && mainThumbnail.thumbnail ? mainThumbnail.thumbnail : null;
    ...
    if (Array.isArray(value)) {
      if (field.isObjectArray) {
        switch (field.isObjectArray) {
          ...
          case "coordinates":
            return (
              value as { latitude: string | null; longitude: string | null }[]
            )
          ...
          case "references":
            return (
              <>
                {(value as { display_name: string; path: string | null }[]).map(
                  (r, idx) => (
